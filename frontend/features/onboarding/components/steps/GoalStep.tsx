...
export default function GoalStep() {
  const selection = useOnboardingStore((s) => s.primaryGoals) || [];
  const setAnswer = useOnboardingStore((s) => s.setAnswer);

  const question = ONBOARDING_QUESTIONS.find((q) => q.id === "goals");
  ...
  const handleToggle = (val: string) => {
    if (selection.includes(val)) {
      ...
      setAnswer("primaryGoals", [...selection, val]);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-xl mx-auto">
        {question.options.map((opt) => {
          const IconComponent = (Icons as any)[opt.icon || "TrendingUp"] || Icons.TrendingUp;
