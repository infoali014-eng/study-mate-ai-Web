...
export default function EducationStep() {
  const selection = useOnboardingStore((s) => s.education);
  const setAnswer = useOnboardingStore((s) => s.setAnswer);
  const nextStep = useOnboardingStore((s) => s.nextStep);

  const question = ONBOARDING_QUESTIONS.find((q) => q.id === "education");
  if (!question || !question.options) return null;

  const handleSelect = (val: string) => {
    setAnswer("education", val);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-xl mx-auto">
        {question.options.map((opt) => {
          ...
          const IconComponent = (Icons as any)[opt.icon || "GraduationCap"] || Icons.GraduationCap;
