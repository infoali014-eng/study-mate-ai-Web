...
export default function LearningStyleStep() {
  const selection = useOnboardingStore((s) => s.learningStyles) || [];
  const setAnswer = useOnboardingStore((s) => s.setAnswer);

  const question = ONBOARDING_QUESTIONS.find((q) => q.id === "learning_style");
  ...
  const handleToggle = (val: string) => {
    if (selection.includes(val)) {
      ...
      setAnswer("learningStyles", [...selection, val]);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-xl mx-auto">
        {question.options.map((opt) => {
          const IconComponent = (Icons as any)[opt.icon || "Copy"] || Icons.Copy;
